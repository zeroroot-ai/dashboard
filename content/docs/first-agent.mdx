...
- You've completed [Install](./install) and `gibson-cli inspect` returns
  successfully for an enrolled agent identity.
- Go installed (the version pinned in the SDK's `go.mod`).

If you haven't enrolled an agent yet, do that first — the dashboard's
deploy wizard issues the credential the SDK needs.

## What an agent looks like

In the SDK, an agent is anything that satisfies the `agent.Agent`
contract from `github.com/zero-day-ai/sdk/agent`. Don't write the
...
dispatch time. See [Observability](./observability) for cost and latency
metrics.
...
[Missions](./missions) for the full event taxonomy.
...
[Tools](./tools), [Plugins](./plugins), [Knowledge graph](./knowledge-graph).
...
- **[Tools](./tools)** — define and call a tool from your agent.
- **[Findings](./findings)** — what `SubmitFinding` does and how
  findings show up in the dashboard.
- **[Knowledge graph](./knowledge-graph)** — querying and writing the
  graph from inside an agent.
- **[Missions](./missions)** — chain your agent with other components
  in a DAG.
- **[Taxonomy](./taxonomy)** and **[Ontology](./ontology)** — extend
