 ...
 * `process.env.NODE_ENV` and its trimmed text is NOT listed for that file
 * in `.permitted-nodeenv.json`.
 *
 * Restricted paths
 * ----------------
 * - `src/lib/auth/**`
 ...
 * audited legitimate hits as `[ { file: "src/lib/auth/x.ts", content: "<the trimmed line>",
...
function loadAllowList() {
  ...
  try {
    ...
    for (const ent of raw.permitted) {
      // Keyed by CONTENT ("file :: trimmed line"), never by line number: a
      // header change or an added import moved the one entry three times
      // before this (159 -> 336 -> 380 -> 383) and each move re-armed the guard
      // against code that had not changed.
      if (typeof ent.file === 'string' && typeof ent.content === 'string') {
        out.add(`${ent.file} :: ${ent.content.trim()}`);
      }
    }
    return out;
  } catch (err) {
