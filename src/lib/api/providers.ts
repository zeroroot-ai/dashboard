...
export function validateProviderName(name: string): { valid: boolean; error?: string } {
  ...
  if (name.length < 2) {
    return { valid: false, error: 'Provider name must be at least 2 characters' };
  }

  if (name.length > 64) {
    return { valid: false, error: 'Provider name must be at most 64 characters' };
  }

  if (!/^[a-z][a-z0-9-]*[a-z0-9]$/.test(name) && !/^[a-z]$/.test(name)) {
    return {
      valid: false,
      error: 'Provider name must start with a letter, contain only lowercase letters, numbers, and hyphens, and end with a letter or number'
    };
  }
...
export function validateApiKeyFormat(
  ...
  switch (providerType) {
    ...
    case 'google':
      if (apiKey.length < 20) {
        return { valid: false, error: 'Google API key appears to be invalid' };
      }
      break;
    // Ollama doesn't require API key
    case 'ollama':
      return { valid: true };
    // Azure and AWS have different auth patterns
    default:
      break;
  }
