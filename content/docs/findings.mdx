...
[`app.zero-day.ai/dashboard/findings`](https://app.zero-day.ai/dashboard/findings)
...
graph (see [Knowledge graph](./knowledge-graph)), linked to:
...
- [Your First Agent](./first-agent) — where `SubmitFinding` fits in
  agent code.
- [Missions](./missions) — `finding.submitted` in the event stream.
- [Knowledge graph](./knowledge-graph) — how findings are linked to
  hosts, services, and attack patterns.
- [Taxonomy](./taxonomy) — the standard finding categories Gibson
  ships with.
- [Ontology](./ontology) — map custom finding categories into CWE,
