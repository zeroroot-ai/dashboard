...
describe('templates/verify', () => {
  const ctx = {
    ...
    verificationUrl: 'https://dash.gibson.zeroroot.ai/verify-email/confirm?token=abc',
    expiresInHours: 24,
  };
...
describe('templates/reset', () => {
  const ctx = {
    ...
    resetUrl: 'https://dash.gibson.zeroroot.ai/reset?token=xyz',
    expiresInHours: 1,
  };
...
describe('templates/claim', () => {
  const ctx = {
    ...
    claimUrl: 'https://dash.gibson.zeroroot.ai/claim?token=q',
    expiresInDays: 7,
    orgName: 'Acme R&D',
    role: 'admin',
...
describe('templates/account-locked', () => {
  const ctx = {
    ...
    resetUrl: 'https://dash.gibson.zeroroot.ai/reset?token=q',
  };
...
describe('templates/billing-rollback', () => {
  const ctx = {
    ...
    currency: 'usd',
    supportEmail: 'billing@zeroroot.ai',
  };
  ...
  it('links the support email in the HTML', () => {
    expect(msg.html).toContain('mailto:billing@zeroroot.ai');
  });
  it('matches snapshot', () => expect(msg).toMatchSnapshot());
});
